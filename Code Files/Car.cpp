...
// Overrides pure virtual from IListable via Vehicle
// Displays car-specific details that Bike wouldn't have
...
// Override: keyword search on brand/model (inherited behavior)
bool Car::matchesSearch(string keyword) const
{
    return Vehicle::matchesSearch(keyword) || transmission == keyword;
}

// Overloaded: search by price range — only exists in Car
// Justified: Car listings often need min/max price filtering
