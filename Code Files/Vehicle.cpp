...
{
    // Check if keyword matches brand or model (case-sensitive for simplicity)
...
// Useful for duplicate listing detection in Marketplace
bool Vehicle::operator==(const Vehicle &other) const
{
    return (price == other.price && brand == other.brand && model == other.model);
}

// + operator: returns combined mileage of two vehicles (for fleet reporting)
...
// Justified: Marketplace deduplication needs brand + model + year without extra getters
